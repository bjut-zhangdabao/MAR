...
class TripletDict:
    ...
    def __init__(self, path_list: List[str]):
        ...
        root_path = os.path.dirname(path_list[0])
        if len(path_list) == 3:  # all_triple_dict
            # self.entities = json.load(open(root_path + '/entities.json', 'r'))
            # self.ent2id = {self.entities[i]['entity_id']: i for i in range(len(self.entities))}
            rels = list(json.load(open(root_path + '/relations.json', 'r')).values())
            self.rel2id = {rels[i]: i for i in range(len(rels))}
            rel_cnt = len(rels)
            for i in range(len(rels)):
                self.rel2id['inverse ' + rels[i]] = i + rel_cnt
            print('------------------------------------------------------------------------------------------------------------')
    ...
    def get_rel_neighbors(self, h: str) -> dict:
        rel_neighbors = {}
        for rel in self.relations:
            if (h, rel) in self.hr2tails.keys():
                rel_neighbors[(h, rel)] = self.hr2tails[(h, rel)]
...
class EntityDict:
    ...
    def __init__(self, entity_dict_dir: str, inductive_test_path: str = None):
        # path = os.path.join(entity_dict_dir, 'img_entities.json')
        path = os.path.join(entity_dict_dir, 'entities1.json')
        # print('======================================================================>', path)
