...
class BertKnowledgeStore(BertPredictor):
    ...
    def _entity_predict_by_examples(self, examples: List[Example]):
        ...
        hr_tensor_list, tail_tensor_list, tail_mm_vector_list, tail_entity_ids, hr_tensor_list_mm = [], [], [], [], []
        ...
        for idx, batch_dict in enumerate(tqdm.tqdm(data_loader, ncols=80)):
            ...
            hr_tensor_list_mm.append(outputs_mm['hr_vector_fusion'])
    ...
    def faiss_index_init(self):
        ...
        if os.path.exists(os.path.join(model_dir, index_file)):
            ...
            if args.task.endswith('_ind'):
                train_examples += load_data(args.valid_path, add_forward_triplet=True, add_backward_triplet=True)
            self.train_examples = train_examples
            print('Ok, now we preparing the knowledge store......please wait several hours......')
            hr_vector, tail_vector, tail_mm_vector, tail_entity_ids, hr_vector_mm = self._entity_predict_by_examples(train_examples)
            hr_numpy = hr_vector.cpu().numpy()
            hr_mm_numpy = hr_vector_mm.cpu().numpy()
            print(faiss.MatrixStats(hr_numpy).comments)
            print('===============>', faiss.MatrixStats(hr_mm_numpy).comments) #为什么不好啊，真烦人啊
            self.index.add(hr_numpy)
            # self.index.add(hr_mm_numpy)
    ...
    def predict_by_entities(self, entity_exs) -> torch.tensor:
        ...
        for idx, batch_dict in enumerate(tqdm.tqdm(data_loader, ncols=80)):
            ...
            if self.use_cuda:
                batch_dict = move_to_cuda(batch_dict)
            outputs = self.model(**batch_dict) #注意，这里是正确的，因为得到的是ent_vectors和ent_vectors_mm
