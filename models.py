...
def build_custommm_model(args) -> nn.Module:
    ...
    return CustomMMWN18Model(args)
    # return CustomATTModel(args)
...
class CustomMMModel(nn.Module, ABC):
    def __init__(self, args):
        ...
        self.liner = nn.Linear(self.vit_config.hidden_size*2, self.vit_config.hidden_size)
    ...
    def forward(self, hr_token_ids, hr_mask, tr_token_ids, tr_mask,
                head_pixel_values,
                head_pixel_values_second,
                tail_pixel_values,
                tail_pixel_values_second,
        ...
        if only_ent_embedding:  # in predict.py predict_by_entities
            return self.predict_ent_embedding(pixel_values=tail_pixel_values, pixel_values_second=tail_pixel_values_second,
                                              token_ids=tail_token_ids, attention_mask=tail_mask)

        # hr_embeds = self._encode_text(self.hr_bert, hr_token_ids, hr_mask)
        tail_embeds_1 = self._encode_image(tail_pixel_values)
        tail_embeds_2 = self._encode_image(tail_pixel_values_second)
        tail_embeds = torch.cat([tail_embeds_1, tail_embeds_2], dim=2)
        tail_embeds = self.liner(tail_embeds)
        head_embeds_1 = self._encode_image(head_pixel_values)
        head_embeds_2 = self._encode_image(head_pixel_values_second)
        head_embeds = torch.cat([head_embeds_1, head_embeds_2], dim=2)
        head_embeds = self.liner(head_embeds)
    ...
    @torch.no_grad()
    def predict_ent_embedding(self, pixel_values, pixel_values_second, token_ids=None, attention_mask=None, **kwargs) -> dict:
        ent_embeds_1 = self._encode_image(pixel_values)
        ent_embeds_2 = self._encode_image(pixel_values_second)
        ent_embeds = torch.cat([ent_embeds_1, ent_embeds_2], dim = 2)
        ent_embeds = self.liner(ent_embeds)
...
class CustomMMWN18Model(nn.Module, ABC):
    def __init__(self, args):
        super().__init__()
        self.args = args
        ...
        self.hr_bert = AutoModel.from_pretrained(args.pretrained_model)
        self.bert = deepcopy(self.hr_bert)
        self.hr_bert_ = deepcopy(self.hr_bert)
        self.bert_ = deepcopy(self.hr_bert)
        # self.bert_t = deepcopy(self.hr_bert)
        self.vit = AutoModel.from_pretrained('./PLMs/vit-base-patch16-224')
        self.vit_ = AutoModel.from_pretrained('./PLMs/vit-base-patch16-224')
        self.prefix = self.args.prefix
        assert self.prefix > 0, "Set args.prefix > 0 to add visual prefix."
        self.vision_mapping = nn.Sequential(
            nn.Linear(self.vit_config.hidden_size, 512),
        ...
        self.vision_mapping_ = nn.Sequential(
            nn.Linear(self.vit_config.hidden_size, 512),
            nn.ReLU(),
            nn.Linear(512, self.config.hidden_size * self.prefix),
            nn.Sigmoid()
        ...
        self.liner = nn.Linear(self.vit_config.hidden_size*2, self.vit_config.hidden_size)
        self.liner_ = nn.Linear(self.vit_config.hidden_size * 2, self.vit_config.hidden_size)
        ...
        # self.linear1_ = nn.Linear(self.vit_config.hidden_size, self.vit_config.hidden_size)
        # self.linear2_ = nn.Linear(self.vit_config.hidden_size, self.vit_config.hidden_size)
    ...
    def _encode_image_(self, entity_pixel_values):
        img_outputs = self.vit_(entity_pixel_values)
        last_hidden_state = img_outputs.last_hidden_state
        img_outputs = last_hidden_state[:, 0, :]
        img_embeds = img_outputs / img_outputs.norm(p=2, dim=-1, keepdim=True)
        img_embeds = self.vision_mapping_(img_embeds).reshape(-1, self.prefix, self.config.hidden_size)
        return img_embeds
    ...
    def _encode_text_hr(self, encoder, input_ids=None, attention_mask=None, inputs_embeds=None, seq_output=None):
        outputs = encoder(input_ids=input_ids,
                          attention_mask=attention_mask,
                          token_type_ids=None,
                          inputs_embeds=inputs_embeds,
    ...
    def _encode_text_t(self, encoder, input_ids=None, attention_mask=None, inputs_embeds=None, seq_output=None):
        outputs = encoder(input_ids=input_ids,
                          attention_mask=attention_mask,
                          token_type_ids=None,
                          inputs_embeds=inputs_embeds,
    ...
    def Wn_(self, X):
        output = self.linear2_(F.relu(self.linear1_(X)))
        # output = self.dropout(output)
    ...
    def emb_with_prefix(self, token_ids, mask, img_feature, seq_output=None):
        input_embeds = self.bert.embeddings(input_ids=token_ids)  # bs, seq_len, dim
        embds_size = input_embeds.size(1)
        input_embeds[:, 1:1 + self.prefix, :] = img_feature  # bs, seq_len, dim
        vision_emb = self._encode_vision(encoder=self.bert, attention_mask=mask,
                                         inputs_embeds=img_feature, seq_output=seq_output)
        prefixed_emb, last_hidden= self._encode_text(encoder=self.bert, attention_mask=mask,
                                         inputs_embeds=input_embeds, seq_output=seq_output)
        last_hidden[:, embds_size - self.prefix: embds_size, :] = vision_emb
        return prefixed_emb, last_hidden

    def _encode_vision(self, encoder, input_ids=None, attention_mask=None, inputs_embeds=None, seq_output=None):
        outputs = encoder(input_ids=input_ids,
                          attention_mask=None,
                          token_type_ids=None,
                          inputs_embeds=inputs_embeds,
    ...
    def emb_with_prefix_(self, token_ids, mask, img_feature, seq_output=None):
        input_embeds = self.hr_bert.embeddings(input_ids=token_ids)  # bs, seq_len, dim
        embds_size = input_embeds.size(1)
        vision_emb = self._encode_vision(encoder=self.hr_bert, attention_mask=mask,
                                         inputs_embeds=img_feature, seq_output=seq_output)
        vision_emb = vision_emb  #要不以以后缀的形式插入到hr_last_hidden
        input_embeds[:, embds_size-self.prefix: embds_size, :] = img_feature  # bs, seq_len, dim
        prefixed_emb, hr_last_hidden= self._encode_text(encoder=self.hr_bert, attention_mask=mask,
                                         inputs_embeds=input_embeds, seq_output=seq_output)
        hr_last_hidden[:, embds_size-self.prefix: embds_size, :] = vision_emb
        return prefixed_emb, hr_last_hidden
    ...
    def forward(self, hr_token_ids, hr_mask, tr_token_ids, tr_mask,
        ...
        tail_embeds = torch.cat([tail_embeds_1, tail_embeds_2], dim=2)
        tail_embeds = self.liner(tail_embeds)
        head_embeds_1 = self._encode_image_(head_pixel_values)
        head_embeds_2 = self._encode_image_(head_pixel_values_second)
        head_embeds = torch.cat([head_embeds_1, head_embeds_2], dim=2)
        head_embeds = self.liner_(head_embeds)
        head_mm_embeds, h_last_hidden= self.emb_with_prefix(token_ids=head_token_ids, mask=head_mask, img_feature=head_embeds)
        tail_mm_embeds, t_last_hidden= self.emb_with_prefix(token_ids=tail_token_ids, mask=tail_mask, img_feature=tail_embeds)
        hr_embeds, hr_last_hidden= self.emb_with_prefix_(token_ids=hr_token_ids, mask=hr_mask, img_feature=head_embeds)  # hr + vis


        # # hr_embeds_visual = torch.concat((head_embeds.mean(dim=1).float(), hr_embeds), dim=1)
        # hr_embeds_last = self.Wn(hr_embeds)
        hr_embeds_last = self._encode_text_hr(encoder=self.hr_bert_, attention_mask=hr_mask, inputs_embeds=hr_last_hidden, seq_output=None)
        ...
        # hr_embeds_last = nn.functional.normalize(hr_embeds_last, dim=1)
        # hr_embeds_last = hr_embeds_last / hr_embeds_last.norm(p=2, dim=-1, keepdim=True)

        # # tail_embeding_and_visual = torch.concat((tail_embeds.mean(dim=1).float(), tail_mm_embeds), dim=1)  #注意一下文本嵌入的前后顺序
        # # tail_embeding_and_visual= self.liner_t(tail_embeding_and_visual)
        # tail_embeding_fusion = self.Wn_(tail_mm_embeds)
        tail_embeding_fusion = self._encode_text_t(encoder=self.bert_, attention_mask=tail_mask, inputs_embeds=t_last_hidden, seq_output=None)
        head_mm_embeds_fusion = self._encode_text_t(encoder=self.bert_, attention_mask=head_mask, inputs_embeds=h_last_hidden, seq_output=None)
        ...
        # tail_embeding_fusion = self.tansformer_fusion_t(tail_embeding_fusion_t_q.unsqueeze(1), tail_embeding_fusion_t_k.unsqueeze(1), tail_embeding_fusion_t_v.unsqueeze(1))
        # tail_embeding_fusion = tail_embeding_fusion.squeeze(1)
        # tail_embeding_fusion = nn.functional.normalize(tail_embeding_fusion, dim=1)
        ...
        return {
            ...
            'head_mm_vector': head_mm_embeds_fusion.float(),
        }
    ...
    def compute_logits_multi_model(self, output_dict: dict, batch_dict: dict) -> dict:
        ...
        hr_logits = hr_vector.mm(tail_vector.t()) * logit_scale
        hr_logits = self._compute_neg_logits(hr_logits, hr_vector, tail_vector, head_vector, batch_dict, logit_scale)
        tail_mm_vector, head_mm_vector = output_dict['tail_mm_vector_fusion'], output_dict['head_mm_vector']
        hr_logits_mm = hr_vector.mm(tail_mm_vector.t()) * logit_scale
        hr_logits_mm = self._compute_neg_logits(hr_logits_mm, hr_vector,
                                                tail_mm_vector, head_mm_vector, batch_dict, logit_scale)
    ...
    def predict_ent_embedding(self, pixel_values, pixel_values_second, token_ids=None, attention_mask=None, **kwargs) -> dict:
        ...
        if token_ids is None or attention_mask is None:
            return {'ent_vectors': ent_embeds.detach()}
        ent_prefix_embeds, last_hidden= self.emb_with_prefix(token_ids=token_ids, mask=attention_mask, img_feature=ent_embeds,
                                                 seq_output=False)
        return {
            'ent_vectors': ent_embeds.mean(dim=1).float(),
            'ent_vectors_mm': ent_prefix_embeds.detach()
