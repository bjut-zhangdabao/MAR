...
def _process_line_wn18rr(line: str) -> dict:
    ...
    if args.task == 'WN9_ind' or args.task == 'WN9':           #here have bug
        head_id = head_id[1:]
        tail_id = tail_id[1:]
...
fb15k_id2desc = {}
fb15k_id2imgpath = {}
fb15k_id2imgpath_second = {}


def _load_fb15k237_wikidata(path: str):
    global fb15k_id2ent, fb15k_id2desc, fb15k_id2imgpath, fb15k_id2imgpath_second
    lines = open(path, 'r', encoding='utf-8').readlines()
    for line in lines:
        fs = line.strip().split('\t')
        ...
        entity_id_trans = 'm.' + entity_id[3:]
        # fb15k_id2ent[entity_id] = (entity_id, name, fb15k_id2desc.get(entity_id, ''),
        #                            fb15k_id2imgpath.get(entity_id, ''))
        fb15k_id2ent[entity_id] = (entity_id, name, fb15k_id2desc.get(entity_id, ''),
                                   fb15k_id2imgpath.get(entity_id_trans, ''), fb15k_id2imgpath_second.get(entity_id_trans, ''))   # 获取不到的图像就用空的填充
...
def _load_fb15k237_imgpath():
    global fb15k_id2imgpath, fb15k_id2imgpath_second
    fb15k237dir = './img_data/FB15k237_ind'
    entities = os.listdir(fb15k237dir)
    # num = 0
    for ent in entities:
        name = 'm.' + ent[2:]
        if os.path.exists(fb15k237dir + '/' +name):
            imgs = os.listdir(fb15k237dir + '/' + ent)
            if Image.open(fb15k237dir + '/' + ent + '/' + imgs[0]):
                fb15k_id2imgpath[name] = fb15k237dir + '/' + ent + '/' + imgs[0]

            # if len(imgs) <= 1:
            #     num += 1
            #     print(fb15k237dir + '/' + ent + '/' + imgs[0], len(imgs))

            if Image.open(fb15k237dir + '/' + ent + '/' + imgs[1]):
                fb15k_id2imgpath_second[name] = fb15k237dir + '/' + ent + '/' + imgs[1]

        else:
            print("no image for entity " + ent)
    print('Load {} entity image paths from {}'.format(len(fb15k_id2imgpath), fb15k237dir))
    print('Load {} entity image paths from =====> {}'.format(len(fb15k_id2imgpath_second), fb15k237dir))
...
def _process_line_fb15k237(line: str) -> dict:
    ...
    _, head, _, _, _ = fb15k_id2ent[head_id]
    _, tail, _, _, _ = fb15k_id2ent[tail_id]
    example = {'head_id': head_id,
               'head': head,
               'relation': relation,
               'tail_id': tail_id,
...
def dump_all_entities(examples, out_path, id2text: dict, id2imgpath: dict, id2imgpath_second: dict):
    id2entity = {}
    relations = set()
        ...
        for ex in examples:
            ...
            if head_id not in id2entity:
                id2entity[head_id] = {'entity_id': head_id,
                                      ...
                                      'entity_imgpath_second': id2imgpath_second[head_id]}
            tail_id = ex['tail_id']
            if tail_id not in id2entity:
                id2entity[tail_id] = {'entity_id': tail_id,
                                      'entity': ex['tail'],
...
def main():
    ...
    if args.task.lower() in ['wn18rr', 'wn18rr_ind', 'wn9', 'wn9_ind', 'wn18']:
        ...
        id2imgpath_second = {k: v[4] for k, v in fb15k_id2ent.items()}
    else:
        assert False, 'Unknown task: {}'.format(args.task)

    dump_all_entities(all_examples,
                      out_path='{}/entities1.json'.format(os.path.dirname(args.train_path)),
                      id2text=id2text,
                      id2imgpath=id2imgpath,
                      id2imgpath_second=id2imgpath_second)
    print('Done')
...
if __name__ == '__main__':
    ...
    if args.task != None:
        ...
        for task in tasks:
            ...
            args.task = task
            args.train_path = f"./data/{args.task}/train.txt"
            args.valid_path = f"./data/{args.task}/valid.txt"
            args.test_path = f"./data/{args.task}/test.txt"
            main()
