...
def eval_single_direction(predictor: BertPredictor,
    ...
    hr_tensor, _, hr_tensor_= predictor.predict_by_examples(examples)  # hr_vector in forward()
    hr_tensor_ = hr_tensor_.to(entity_tensor.device)
    ...
    if args.knn_topk > 0:
        topk_scores, topk_indices, metrics, ranks = compute_metrics_search_knn_lamda(hr_tensor=hr_tensor, # hr_tensor=hr_tensor,
                                                                                     entities_tensor=entity_tensor,
                                                                                     target=target, examples=examples,
                                                                                     batch_size=batch_size,
                                                                                     predictor=predictor)
    else:
        topk_scores, topk_indices, metrics, ranks = compute_metrics(hr_tensor=hr_tensor, entities_tensor=entity_tensor, # hr_tensor=hr_tensor,
                                                                    target=target, examples=examples,
...
def predict_with_knowledge_store():
    ...
    entity_tensor, entity_tensor_mm = predictor.predict_by_entities(  #这是啥子玩意？应该是entity的所有实体的多模态表示
        entity_dict.entity_exs)  # obtain entity embeddings
